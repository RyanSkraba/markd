...
/** An "invisible" top-level markdown element that only serves as a container for the others. This is useful if there is
  * text before the Header(1) or if there are multiple Header(1) in a page.
  *
  * @param mds
  *   The internal subsections and parsed [[Markd]] elements.
  */
case class Doc(mds: Markd*) extends MultiMarkd[Markd] {

  type Self = Doc

  override def copyMds(newMds: Seq[Markd]): Self = Doc(newMds: _*)
...
object Doc {
  ...
  // def apply(sub: Markd*): Doc =  new Doc( sub)
  ...
  def parse(content: String, cfg: ParserCfg = new ParserCfg()): Doc = {
    ...
    Doc(organizeHeaderContents(pass4).mds: _*)
  }
}
